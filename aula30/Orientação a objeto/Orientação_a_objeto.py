while True:
            ...
            if operacao_teoria =="01":
                ...
                print("##################################call: Permite que um objeto seja chamado como uma função.##################################")
                print("O método __call__ em Python permite que um objeto seja chamado como uma função. Isso pode ser útil para criar classes que se comportam de maneira semelhante a funções embutidas ou para implementar padrões de design específicos.")
                continuar = input("Deseja printar o código abaixo? (s/n) ")
                if continuar.lower()=="s":
                    class ConversorTemperatura:
                        def __init__(self, unidade_origem, unidade_destino):
                            self.unidade_origem = unidade_origem
                            self.unidade_destino = unidade_destino

                        def converter(self, valor):
                            if self.unidade_origem == "Celsius" and self.unidade_destino == "Fahrenheit":
                             return valor * 1.8 + 32
                            elif self.unidade_origem == "Fahrenheit" and self.unidade_destino == "Celsius":
                             return (valor - 32) / 1.8
                            else:
                                raise ValueError("Unidades inválidas")

                        # Definindo o método __call__
                        def __call__(self, valor):
                            return self.converter(valor)

                        # Criando um conversor Celsius para Fahrenheit
                        conversor_c_f = ConversorTemperatura("Celsius", "Fahrenheit")

                        # Usando o conversor como uma função
                        temperatura_f = conversor_c_f(20)  # temperatura_f = 68.0

                        print(f"{20} graus Celsius equivalem a {temperatura_f} graus Fahrenheit")    
                print("##################################getitem: Define como os elementos de um objeto podem ser acessados através de indexação.##################################")
                continuar = input("Deseja printar o código abaixo (s/n)")
                if continuar.lower()=="s":
                    class MinhaLista:
                        ...
                        # Acessando elementos
                        primeiro_elemento = minha_lista[0]  # primeiro_elemento = 1
                        terceiro_elemento = minha_lista[2]  # terceiro_elemento = 3

                        # Acesso inválido
                        #ultimo_elemento = minha_lista[3]  # IndexError: Índice fora do range
                print("##################################setitem: Define como os elementos de um objeto podem ser modificados através de atribuição.##################################")
                continuar = input("Deseja printar o código abaixo (s/n)")
                if continuar.lower()=="s":
                    class MinhaLista:
                        def __init__(self, elementos):
                        ...
                        minha_lista[1] = 5  # Substitui o elemento de índice 1 por 5 (originalmente era 2)

                        # Acesso após modificação
                        primeiro_elemento = minha_lista[0]  # primeiro_elemento = 1
                        novo_segundo_elemento = minha_lista[1]  # novo_segundo_elemento = 5

                        # Acesso inválido para atribuição
                        # minha_lista[3] = 10  # IndexError: Índice fora do range
                print("##################################delitem: Define como os elementos de um objeto podem ser removidos através da instrução del.##################################")
                continuar = input("Deseja printar o código abaixo (s/n)")
                if continuar.lower()=="s":
                    class MinhaLista:
                        def __init__(self, elementos):
                        ...
                        # Removendo elemento
                        del minha_lista[1]  # Remove o elemento de índice 1 (originalmente era 2)

                        # Acesso após remoção
                        primeiro_elemento = minha_lista[0]  # primeiro_elemento = 1
                        restante_elemento = minha_lista[1]  # restante_elemento = 3 (originalmente era 2)

                        # Acesso inválido para remoção
                        # del minha_lista[3]  # IndexError: Índice fora do range
                print("##################################iter: Retorna um iterador que permite iterar sobre os elementos do objeto.##################################")
                continuar = input("Deseja printar o código abaixo (s/n)")
                if continuar.lower()=="s":
                    class ListaCompras:
                        ...
                        def __iter__(self):
                            # Criar um índice interno para acompanhar a iteração
                            self.indice = 0

                            # Retornar o 'self' como um iterador (padrão em Python)
                            return self

                        def __next__(self):
                            # Verificar se ainda há itens na lista
                            if self.indice < len(self.itens):
                            # Obter o item e a quantidade atuais
                             item, quantidade = self.itens[self.indice]

                            # Atualizar o índice para o próximo item
                             self.indice += 1

                            # Retornar uma tupla contendo o item e a quantidade
                             return item, quantidade

                            # Se não houver mais itens, levantar a exceção StopIteration
                            else:
                             raise StopIteration()

                        # Criando uma lista de compras
                        lista_compras = ListaCompras([("Maçã", 3), ("Leite", 1), ("Ovos", 6)])

                        # Iterando na lista de compras
                        for item, quantidade in lista_compras:
                         print(f"{quantidade}x {item}")
                print("##################################next: Define como o próximo elemento do iterador deve ser retornado.##################################")
                continuar = input("Deseja printar o código abaixo (s/n)")
                if continuar.lower()=="s":
                    class SequenciaPares:
                        ...
                        def __iter__(self):
                            # Criar um índice interno para acompanhar a iteração
                            self.indice = self.inicio - 2

                            # Retornar o 'self' como um iterador (padrão em Python)
                            return self

                        def __next__(self):
                            # Verificar se ainda há números pares na sequência
                            if self.indice < self.fim:
                            # Obter o próximo número par
                             numero_par = self.indice + 2

                            # Atualizar o índice para o próximo número par
                             self.indice += 2

                            # Retornar o número par
                             return numero_par

                            # Se não houver mais números pares, levantar a exceção StopIteration
                            else:
                             raise StopIteration()

                        # Criando uma sequência de números pares
                        sequencia_pares = SequenciaPares(2, 10)

                        # Iterando na sequência de números pares
                        for numero_par in sequencia_pares:
                         print(numero_par)
                print("##################################getattr: Define como atributos que não existem no objeto podem ser acessados.##################################")
                continuar = input("Deseja printar o código abaixo(s/n)")
                if continuar.lower()=="s":
                    class MinhaClasse:
                        ...
                        # Criando uma instância
                        obj = MinhaClasse("valor")

                        # Acessando atributo existente
                        print(obj.atributo1)  # Saída: valor

                        # Acessando atributo ausente
                        # print(obj.atributo2)  # Erro: AttributeError: Atributo 'atributo2' não encontrado
                print("##################################setattr:Define como atributos do objeto podem ser modificados.##################################")
                continuar = input("Deseja printar o código abaixo(s/n)")
                if continuar.lower()=="s":
                    class Pessoa:
                        def __init__(self, nome):
                        ...
                        # Atribuindo valor usando notação de ponto
                        pessoa.idade = 30  # Válido

                        # Atribuindo valor usando setattr
                        setattr(pessoa, "sobrenome", "Silva")

                        # Acessando atributos
                        print(f"Nome: {pessoa.nome}")
                        print(f"Sobrenome: {pessoa.sobrenome}")  # Acessando atributo definido por setattr
                print("##################################getattribute: Intercepta o acesso a qualquer atributo do objeto, permitindo que você personalize o comportamento de leitura.##################################")
                continuar = input("Deseja printar o código abaixo(s/n)")
                if continuar.lower()=="s":
                    class Conta:
                        def __init__(self, saldo):
                        ...
                        def __getattribute__(self, name):
                            # Interceptar acesso ao atributo 'saldo'
                            if name == 'saldo':
                             print(f"Consultando saldo...")
                             return super().__getattribute__(name)  # Acesso ao atributo original
                            else:
                        ...
                        # Acessando saldo (interceptado por __getattribute__)
                        print(f"Seu saldo é: R$ {conta.saldo}")
                print("##################################setattribute: Intercepta a modificação de qualquer atributo do objeto, permitindo que você personalize o comportamento de escrita.##################################")
                continuar = input("Deseja printar o código abaixo(s/n)") 
                if continuar.lower()=="s":
                    class Pessoa:
                        def __init__(self, nome):
                        ...
                        # Definir atributo usando notação de ponto (forma padrão)
                        pessoa.idade = 30

                        # Definir atributo usando setattr (útil para cenários dinâmicos)
                        atributo_dinamico = "profissao"
                        setattr(pessoa, atributo_dinamico, "Engenheira de Software")

                        # Acessando atributos
                        print(f"Nome: {pessoa.nome}")
                        print(f"Idade: {pessoa.idade}")
                        print(f"Profissão: {pessoa.profissao}")  # Acessando atributo definido usando setattr                   
                print("##################################repr:Define uma representação textual mais detalhada do objeto, utilizada para fins de depuração e introspecção.##################################")   
                continuar = input("Deseja printar o código abaixo(s/n)") 
                if continuar.lower()=="s":
                    class ContaCorrente:
                        ...
                        def __repr__(self):
                            return f"ContaCorrente(titular='{self.titular}', saldo={self.saldo})"

                        conta = ContaCorrente("João Silva", 1000.00)
                        print(repr(conta))  # Saída: ContaCorrente(titular='João Silva', saldo=1000.00)
                print("##################################eq:Define se dois objetos são considerados iguais.##################################")  
                continuar = input("Deseja printar o código abaixo(s/n)") 
                if continuar.lower()=="s":
                    class Pessoa:
                        ...
                        pessoa3 = Pessoa("Maria Oliveira", 25)

                        print(pessoa1 == pessoa2)  # Saída: True
                        print(pessoa1 == pessoa3)  # Saída: False
                print("##################################ne:Define se dois objetos não são considerados iguais.##################################")  
                continuar = input("Deseja printar o código abaixo(s/n)") 
                if continuar.lower()=="s":
                    class Pessoa:
                        def __init__(self, nome, idade):
                        ...
                        def __ne__(self, outra_pessoa):
                            return not self == outra_pessoa
                print("##################################lt:Define se um objeto é menor que outro.##################################")  
                continuar = input("Deseja printar o código abaixo(s/n)") 
                if continuar.lower()=="s":
                    class Ponto2D:
                        ...
                        def __lt__(self, outro_ponto):
                            if self.x < outro_ponto.x:
                             return True
                            elif self.x == outro_ponto.x and self.y < outro_ponto.y:
                             return True
                            else:
                             return False
                        ...
                        print(ponto1 < ponto3)  
                        print(ponto2 < ponto3)  
                print("##################################gt:Define se um objeto é maior que outro.##################################")  
                continuar = input("Deseja printar o código abaixo(s/n)") 
                if continuar.lower()=="s":
                    class Ponto2D:
                        def __init__(self, x, y):
                    ...
                    print(ponto1 < ponto3)  
                    print(ponto2 < ponto3)  
                print("##################################le:Define se um objeto é menor ou igual que outro.##################################")  
                continuar = input("Deseja printar o código abaixo(s/n)") 
                if continuar.lower()=="s":
                    class Ponto2D:
                        def __init__(self, x, y):
                        ...
                        def __le__(self, outro_ponto):
                            return self < outro_ponto or self == outro_ponto
                print("##################################ge:Define se um objeto é maior ou igual que outro.##################################")  
                continuar = input("Deseja printar o código abaixo(s/n)") 
                if continuar.lower()=="s":
                    class Produto:
                        def __init__(self, nome, preco, quantidade):
                        ...
                        def __ge__(self, outro_produto):
                            if self.preco > outro_produto.preco:
                             return True
                            elif self.preco == outro_produto.preco and self.quantidade >= outro_produto.quantidade:
                             return True
                            else:
                             return False

                        # Criando alguns produtos
                        produto1 = Produto("Camisa", 50.00, 10)
                        produto2 = Produto("Calça", 75.00, 5)
                        produto3 = Produto("Camisa", 50.00, 15)

                        # Comparando produtos
                        print(produto1 >= produto2)  
                        print(produto1 >= produto3)  
                        print(produto2 >= produto3)  
                print("##################################floordiv:Retorna a parte inteira do resultado da divisão.##################################")  
                continuar = input("Deseja printar o código abaixo(s/n)") 
                if continuar.lower()=="s":
                    class Pagina:
                        ...
                        def __floordiv__(self, outra_pagina):
                            if isinstance(outra_pagina, Pagina):
                                total_texto = len(self.texto) + len(outra_pagina.texto)
                                paginas_necessarias = total_texto // 200  # 200 palavras por página
                                return paginas_necessarias
                            else:
                                raise TypeError("Divisão de piso só é permitida entre objetos Pagina")

                    # Criando algumas páginas
                    pagina1 = Pagina(1, "Texto da página 1")
                    pagina2 = Pagina(2, "Texto da página 2")
                    pagina3 = 1000  # Objeto não Pagina

                    # Dividindo páginas
                    print(pagina1 // pagina2)  # Saída: 1 (1 página é suficiente para o texto combinado)
                    print(pagina2 // pagina1)  # Saída: 1 (1 página é suficiente para o texto combinado)
                    print(pagina1 // pagina3)  # Saída: TypeError: Divisão de piso só é permitida entre objetos Pagina
                print("##################################enter:Define o que deve ser executado quando um objeto é utilizado com a instrução with./exit: Define o que deve ser executado quando se sai do bloco with, independentemente de exceções.##################################")  
                continuar = input("Deseja printar o código abaixo(s/n)") 
                if continuar.lower()=="s":
                    class GerenciadorDeArquivo:
                        ...
                        def __enter__(self):
                            try:
                                ...
                                print(f"Arquivo {self.nome_arquivo} aberto com sucesso!")
                                return self.arquivo  # Retorna o objeto arquivo para uso no bloco
                            except Exception as e:
                                print(f"Falha ao abrir o arquivo {self.nome_arquivo}: {e}")
                    ...
                    # Abrindo e usando o arquivo
                    with GerenciadorDeArquivo("arquivo.txt") as arquivo:
                        conteudo = arquivo.read()
                        print(f"Conteúdo do arquivo:\n{conteudo}")
                print("##################################format:permite que você defina como as instâncias de sua classe serão formatadas quando usadas com métodos de formatação de string como str.format() e f-strings. Isso lhe concede controle sobre a representação de saída de seus objetos.##################################")  
                continuar = input("Deseja printar o código abaixo(s/n)") 
                if continuar.lower()=="s":
                    class Ponto3D:
                        ...
                        def __format__(self, formato):
                            if formato is None:
                                formato = ""
                            return f"({self.x:{formato}}, {self.y:{formato}}, {self.z:{formato}})"

                    # Criando pontos 3D
                    ponto1 = Ponto3D(1.5, 2.3, 4.1)
                    ponto2 = Ponto3D(5, 10, 15)

                    # Usando f-strings
                    print(f"Ponto 1: {ponto1}")  # Saída: Ponto 1: (1.5, 2.3, 4.1)
                    print(f"Ponto 2: {ponto2:.2f}")  # Saída: Ponto 2: (5.00, 10.00, 15.00)

                    # Usando método format()
                    print("Ponto 1: {}".format(ponto1))  # Saída: Ponto 1: (1.5, 2.3, 4.1)
                    print("Ponto 2: {:.2e}".format(ponto2))  # Saída: Ponto 2: 5.000000e+00, 1.000000e+01, 1.500000e+01
                print("##################################hash:é responsável por definir como os objetos de sua classe são armazenados em um hash. O hash é um conceito fundamental usado em dicionários, conjuntos e outras estruturas de dados para armazenamento e recuperação eficientes.##################################")  
                continuar = input("Deseja printar o código abaixo(s/n)") 
                if continuar.lower()=="s":
                    class Ponto2D:
                        def __init__(self, x, y):
                    ...
                    # Usando em dicionário
                    meu_dicionario = {ponto1: "Ponto 1", ponto2: "Ponto 2"}

                    # Verificando se os pontos estão no dicionário (usa hash internamente)
                    print(ponto3 in meu_dicionario)  # Saída: False (ponto3 tem coordenadas diferentes)

                    # Comparando pontos
                    print(ponto1 == ponto2)  # Saída: True
