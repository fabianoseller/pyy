    ...
    if form.validate_on_submit():
        ...
        try:
            with get_db_connection() as connection:
                ...
                connection.commit()
            return render_template('sucesso.html', message="Mensagem enviada com sucesso!")
        except mysql.connector.Error as err:
...
@app.route('/sucesso')
def sucesso():
    return render_template('sucesso.html', message="Operação realizada com sucesso!")
    ...
    if 'loggedin' in session:
        try:
            with get_db_connection() as connection:
                cursor = connection.cursor(dictionary=True)
                cursor.execute('''
                    SELECT uc.users_id, u.username, uc.contatos_id, c.nome, uc.situacao
