...
app = Flask(__name__)
app.config['SECRET_KEY'] = 'outra-chave-quasesecreta'
...
def contato():
    ...
    if form.validate_on_submit():
        ...
        try:
            ...
            conectividade =conecta_no_banco_de_dados()
            # A linha conectividade = conecta_no_banco_de_dados() tenta estabelecer uma conexão com o banco de dados usando a função conecta_no_banco_de_dados().
            # Essa função, é responsável por lidar com os detalhes da conexão.
            
            cursor = conectividade.cursor()
            sql = "INSERT INTO contatados (nome, email, mensagem) VALUES (%s, %s, %s)"
            ...
            # Um cursor para executar comandos SQL é criado usando cursor = conectividade.cursor().
...
@app.route('/contatados')
def pagina_contatados():
    conectividade =conecta_no_banco_de_dados()
    cursor = conectividade.cursor()
    cursor.execute('SELECT * FROM contatados')
    contatados = cursor.fetchall()
    return render_template("contatados.html", contatados=contatados)
